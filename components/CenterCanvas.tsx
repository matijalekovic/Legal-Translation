...
const CenterCanvas: React.FC<CenterCanvasProps> = ({ selectedDocument }) => {
  ...
  if (!selectedDocument) {
    return (
      <div className="flex-grow h-[calc(100vh-56px)] bg-lightGray-200 flex flex-col items-center justify-center p-4">
        <div className="w-16 h-16 bg-lightGray-100 rounded-full flex items-center justify-center mb-3 border border-lightGray-300">
          <FileText className="w-8 h-8 text-slate-300" />
        </div>
        <h3 className="text-sm text-slate-400 font-normal">Select a document to preview</h3>
      </div>
    );
  }
  ...
  return (
    <div className="flex-grow h-[calc(100vh-56px)] bg-lightGray-200 flex flex-col relative overflow-hidden">

      {/* Sticky Preview Header */}
      <div className="h-10 bg-white border-b border-lightGray-200 flex items-center justify-between px-3 shrink-0 z-10 shadow-sm">
        <div className="flex items-center space-x-1.5 overflow-hidden">
          {isTranslated ? (
            <Languages className="w-3.5 h-3.5 text-success-green shrink-0" />
          ) : (
            <FileText className="w-3.5 h-3.5 text-profBlue-800 shrink-0" />
          )}
          <span className="text-xs font-medium text-navy-900 truncate max-w-[300px]">{name}</span>
          {isTranslated && (
            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-success-bg text-success-text border border-success-green/20">
              Translated
            </span>
          )}
        </div>
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center bg-lightGray-100 rounded-md border border-lightGray-300">
            <button
              ...
              className="p-1 hover:bg-white rounded-l-md transition-colors border-r border-lightGray-300"
            >
      ...
      {/* Preview Area */}
      <div className="flex-grow overflow-auto p-4 custom-scrollbar bg-[#e2e8f0] flex justify-center">
        <div
